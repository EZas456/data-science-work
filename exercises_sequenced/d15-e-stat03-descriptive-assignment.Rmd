...
We'll use the quartiles to define the *innerquartile range*. First, the `quantile()` function computes quantiles of a sample. For example:
...
__q3__ Using the function `quantile`, compute the *innerquartile range*; this is the difference between the $75%$ and $25%$ quantiles.
...
*Spread* is the concept of how tightly or widely data are *spread out*. There are two primary measures of spread: the *standard deviation*, and the *innerquartile range*.
...
__q4__ Using the code from q2 as a starting point, compute the standard deviation (`sd()`) and innerquartile range (`IQR()`), and rank the top five carriers, this time by sd and IQR. Report your observations on which carriers are in both lists, and which are different. Also note and comment on which carrier (among your top-ranked) has the largest difference between `sd` and `IQR`.
...
The [Spearman correlation](https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient) is often denoted by $\rho_{XY}$, and is actually defined in terms of $r_{XY}$, but with the ranks ($1$ to $n$) rather than the values $X_i, Y_i$.
...
One last point about correlation: The mean is to Pearson correlation as the median is to Spearman correlation; the latter are robust to outliers.

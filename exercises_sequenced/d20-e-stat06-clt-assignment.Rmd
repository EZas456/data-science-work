---
title: "Statistics: The Central Limit Theorem and Confidence Intervals"
...
This is approximately `1.96` when seeking a $95%$ confidence level. Since the normal distribution is symmetric, we can use the same value `q95` with a negative sign for the appropriate lower quantile.
...
The following code estimates the frequency with which each interval includes the true mean; this quantity is called *coverage*, and should match the nominal $95%$ we selected above.
...
See Note [2] for an animation that helps explain how confidence intervals are constructed.

__q1__ Using the CLT, approximate a confidence interval for the population mean using the sample `z_q1`.
...
[4] The process of making decisions about what to believe about reality based on data is called [hypothesis testing](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing).

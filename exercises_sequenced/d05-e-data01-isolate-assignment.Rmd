...
__q2__ Re-arrange the columns to place `dest, origin, carrier` at the front, but
...
__q3__ Fix the following code. What is the mistake here? What is the code trying
to accomplish?
...
```{r q4-task}
#"BOS" <- "LGA"
flights %>% filter(dest == "BOS", origin == "LGA")
```
Gotta have those quotations. I can only assume that the user was looking for flights from LGA to BOS but assigned the value "LGA" to the object BOS.
...
__q6__ Sort the flights in *descending* order by their `air_time`. Bring
`air_time, dest` to the front. What can you tell about the longest flights?
...
df_q6
```
We see that the longest flights are to Honolulu which is indeed, very far away. Duration is consistent with my experience going home to see family.

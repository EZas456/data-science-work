...
**Observations**:
  ...
  - Is there “one true value” for the material properties of Aluminum?
    ...
    Aluminum. There is a distribution centered about a mean.
  - How many aluminum alloys were tested? How do you know? There was one
    alloy tested (al\_24st). The summary does not give multiple
    characters.
  - What angles were tested? Angles were 0, 45 and 90 degrees.
  - What thicknesses were tested? 0.022, 0.032, 0.064, and 0.081
  - Are E and mu always the same for a given thickness? No; we see a
    variety of values for `E` and `mu`.
...
**Observations**:
  ...
  - For each thickness, there are multiple different mu and E values.
    For the answer to be yes, there would have to be a 1-to-1 mapping
    from mu or E to thickness.
...
**Observations**:
  ...
  - This graph does tell us that the 0.081 has a slightly different E
    from the other thicknesses (by about a factor of 5%). However, in
    order for the data to support the claim that amount of material
    present *does* affect E, we would like to see a trend where the
    thickest samples (0.081) performs at some level, the thinnest
    performs at a different level and the two thicknesses between
    perform between those levels (in order of thickness). This is not
    the case, since the second thickest samples are actually closer to
    the thinnest sample on average than the third thickest. Furthermore,
    it is a little weird that the value of E (normalized stiffness) is
    *lower* for the thickest samples.
...
![](c03-stang-assignment_files/figure-gfm/q4-task2-1.png)<!-- -->
...
![](c03-stang-assignment_files/figure-gfm/q4-task2-2.png)<!-- -->

**Observations**:

  - Here, we see that the samples’ distributions are overlapping very
    little for mu in thickness of 0.081. In E we see the same grouping
    of the thickest samples. However, we cannot draw conclusions about
    differences between the other samples since they overlap so much.
